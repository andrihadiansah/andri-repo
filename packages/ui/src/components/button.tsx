  ...
  (
    ...
    return (
      <Comp
        ...
        {...props}
      >
        {withIcon && <span className="">{withIcon}</span>}
        {withIndicator && (
          <span className="flex items-center h-3 w-3 mr-2">
            <span
              className={`animate-ping absolute inline-flex h-3 w-3 rounded-full ${withIndicator} opacity-75`}
            ></span>
            <span
              className={`relative inline-flex rounded-full h-3 w-3 ${withIndicator}`}
            ></span>
          </span>
        )}
        {props.children}
